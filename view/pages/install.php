...
<head>
    ...
    <link type="image/x-icon" rel="shortcut icon" href="/view/style/newblog.png">
...
<body style="background-image: url(/view/style/background.jpg)">

    <?php
	$server = $_SERVER['DOCUMENT_ROOT'];
	$getEtape = file_get_contents($server . '/model/data/settings/installcheck.txt');

	if ($getEtape == "ok") {
		echo '<div class="annonce">Votre CMS est déjà installé !</div>';
	} elseif ($getEtape == "") {
		$installEtape = fopen($server . '/model/data/settings/installcheck.txt', "w+");
		...
		if (isset($_POST['idconfig'])) {
			...
			if (isset($_POST['adduser']) and isset($_POST['addpass'])) {
				...
				$userfile = fopen($server . '/model/data/settings/user.txt', "w+");

				$passfile = fopen($server . '/model/data/settings/pass.txt', "w+");
		...
		if (isset($_POST['vfirstpost'])) {
			...
			if (isset($_POST['firstpost'])) {
				...
				$postfile = fopen($server . '/model/data/settings/post.txt', "a+");
		...
		if (isset($_POST['vnblog'])) {
			...
			if (isset($_POST['nblog'])) {
				...
				$namefile = fopen($server . '/model/data/settings/name.txt', "w+");
		...
		if (isset($_POST['vdescblog'])) {
			...
			if (isset($_POST['descblog'])) {
				...
				$descfile = fopen($server . '/model/data/settings/description.txt', "w+");
	...
	?>
    <center>
        <iframe width="50%" height="350px" src="" frameborder="0" allowFullScreen="">Bientôt » Vidéo d'installation du
            CMS !</iframe>
    </center>