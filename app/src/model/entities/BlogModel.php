		...
		try {
			...
			if (is_null(Model::getPdo())) {
				...
				try {
					...
					$installFile = file_get_contents(__DIR__ . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'database' . DIRECTORY_SEPARATOR . '1-install_tables.sql');
					// If the file could not be read
					if (!$installFile) {
						// Throw an error that will be caught below
						throw new Exception('The SQL tables creation file could not be read!');
					}
					// Execute the content of the SQL file
					if (Model::getPdo()->exec($installFile) === false) {
						// If an error occurs, throw an exception that will be caught below
						throw new Exception('The SQL tables creation file could not be executed!');
					}

					// Get the content of the SQL installation file
					$installUsersFile = file_get_contents(__DIR__ . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . 'database' . DIRECTORY_SEPARATOR . '2-install_users.sql');
					// If the file could not be read
					if (!$installUsersFile) {
						// Throw an error that will be caught below
						throw new Exception('The SQL user creation file could not be read!');
					}
					// Execute the content of the SQL file
					if (Model::getPdo()->exec($installUsersFile) === false) {
						// If an error occurs, throw an exception that will be caught below
						throw new Exception('The SQL user creation file could not be executed!');
					}
			...
			if (is_null(Model::getPdo())) {
				...
				if (empty($bgURL)) {
					...
					$stmt = Model::getPdo()->prepare(
						"INSERT INTO nb_blog (blog_name, description, id_user_owner)
					...
					$stmt = Model::getPdo()->prepare("INSERT INTO nb_blog (blog_name, description, background_url, id_user_owner)
		...
		try {
			...
			if (is_null(Model::getPdo())) {
				...
				$stmt = Model::getPdo()->prepare(
					"SELECT * 
					FROM nb_blog 
					WHERE nb_blog.blog_name = :blogname"
				);
				if (!$stmt) {
					// If the query could not be prepared
					throw new Exception('The blog data retrieval query could not be prepared!');
