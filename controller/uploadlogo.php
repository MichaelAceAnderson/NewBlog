...
if (!empty($_FILES)) {
	...
	if ($errorLogo != 0 || !$tempLogo) {
		...
		if (move_uploaded_file($tempLogo, $server . ('/view/style/' . $nomLogo . ''))) {
			$getlogo = file_get_contents($server . '/model/data/settings/logo.txt');
			if ($getlogo != "") {
				unlink($server . $getlogo);
			}
			echo '<p style="color: white;"> Chargement du fichier ' . $nomLogo . ' terminé !</p>';
			$openlogofile = fopen($_SERVER['DOCUMENT_ROOT'] . "/model/data/settings/logo.txt", "w+");


			if (fwrite($openlogofile, "/view/style/$nomLogo")) {
