...
if (!empty($_FILES)) {
    ...
    if ($error != 0 || !$tempRep) {
        ...
        if (move_uploaded_file($tempRep, $rep . ('/model/data/media/' . $nomFichier . ''))) {
            echo '<p style="color: white;"> Chargement du fichier ' . $nomFichier . ' terminé !</p>';
            $postfile = fopen("settings/post.txt", "a+");

            if (fwrite($postfile, "<img src=\"/model/data/media/$nomFichier\">\n")) {
        ...
        if (move_uploaded_file($tempRep, $rep . ('/model/data/media/' . $nomFichier . ''))) {
            ...
            if (fwrite($postfile, "<video controls=\"\" preload=\"\" src=\"/model/data/media/$nomFichier\">\n")) {
